...
namespace Polyrific.Catapult.Engine.UnitTests.Commands
{
    public class ConfigCommandTests
    {
        ...
        public ConfigCommandTests(ITestOutputHelper output)
        {
            ...
            _engineConfig.Setup(c => c.Load()).Returns(Task.CompletedTask);
            _engineConfig.Setup(c => c.Configs).Returns(_configs);
            _engineConfig.Setup(c => c.GetValue(It.IsAny<string>())).Returns((string configName) =>
                _configs.ContainsKey(configName) ? _configs[configName] : "");
            _engineConfig.Setup(c => c.SetValue(It.IsAny<string>(), It.IsAny<string>()))
            ...
            _engineConfig.Setup(c => c.RemoveValue(It.IsAny<string>()))
                .Callback((string configName) => _configs.Remove(configName));
            _engineConfig.Setup(c => c.Save()).Returns(Task.CompletedTask);
        }
        ...
        public void ConfigGet_GetAll_Execute_ReturnsItems()
        {
            ...
            Assert.StartsWith("Available Engine configs:", message);
        }
