...
Create a folder `Helpers` and create a class `CommandHelper.cs` inside of it. The CommandHelper static class will have a static method named `ExecuteShellCommand` that execute a node command with arguments.
